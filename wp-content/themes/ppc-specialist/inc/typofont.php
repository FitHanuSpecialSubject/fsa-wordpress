...
function ppc_specialist_custom_styles($custom) {
	...
	if ( $ppc_specialist_headings_font ) {
		...
		$font_pieces = explode(":", $ppc_specialist_headings_font);
	...
	if ( $ppc_specialist_body_font ) {
		...
		$font_pieces = explode(":", $ppc_specialist_body_font);
	...
	wp_add_inline_style( 'ppc-specialist-style', $custom );

}
add_action( 'wp_enqueue_scripts', 'ppc_specialist_custom_styles' );

//Sanitizes Fonts
function ppc_specialist_sanitize_fonts( $input ) {
	$valid = array(
		'' => 'Select',
